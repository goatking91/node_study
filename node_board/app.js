...
app.post('/write', function(req, res) {
	...
	conn.query('insert into nodeboard(writer,title,content,pwd,hit,regdate) values(?,?,?,?,0,now())', 
...
app.get('/list', function(req, res) {
	var sql = "select num,writer,title,content,pwd,hit,DATE_FORMAT( regdate,'%Y-%m-%d %H:%i:%s' ) regdate from nodeboard order by num desc";
...
app.get('/read/:num', function(req, res) {
	...
	conn.query('select * from nodeboard where num=?', [num], function(err, results) {
...
app.get('/update/:num', function(req, res) {
	...
	conn.query('select * from nodeboard where num=?', [num], function(err, results) { // 글 번호로 검색
...
app.post('/update', function(req, res) {
	...
	conn.query('update nodeboard set writer=?, title=?, content=? where num=? and pwd=?', 
...
app.post('/delete', function(req, res) {
	...
	conn.query('delete from nodeboard where num=? and pwd=?', 
